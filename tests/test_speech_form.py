...
SEOKBINGGO = [
    ("ko", "바닥 배수로는 5도 기울여", "바닥 배수로는 오 도 기울여"),
    ("ko", "녹는 양이 0.4%뿐이에요.", "녹는 양이 영 점 사 퍼센트뿐이에요."),
    ("ko", "짚이 있어도 18%가 녹아요.", "짚이 있어도 십팔 퍼센트가 녹아요."),
    ("ko", "한겨울 12cm 넘게 언", "한겨울 십이 센티미터 넘게 언"),
    ("ko", "300년 전에 이미", "삼백 년 전에 이미"),
...
KO_SAMPLES = [
    ("1cm", "일 센티미터"),                       # 사람이 든 예 그대로
    ("1592년, 임진왜란이", "천오백구십이 년, 임진왜란이"),   # 연 (57%)
    ("1970년대", "천구백칠십 년대"),
    ("2006년엔", "이천육 년엔"),
    ("8개월", "팔 개월"),
    ("6월", "유월"),                              # 음이 준다 — 육월이 아니다
    ...
    ("20여만 개를", "이십여만 개를"),
    ("6mm 구멍을", "육 밀리미터 구멍을"),            # 라틴 SI (8.1%)
    ("5.74km", "오 점 칠사 킬로미터"),
    ("8,000 m³였죠.", "팔천 세제곱미터였죠."),        # 천 단위 구분 + 띄어 쓴 라틴
    ("0.11헤르츠.", "영 점 일일 헤르츠."),           # 한글 단위어 (5.8%)
    ("9초마다", "구 초마다"),                       # 시간 (3.5%)
    ("8시 55분", "여덟 시 오십오 분"),               # 시각의 시는 고유어, 분은 한자어
    ("27단으로 쌓아", "이십칠 단으로 쌓아"),          # 조수사가 한자어를 고른다
    ("40장으로", "마흔 장으로"),                    # 〃 고유어
    ("33갈래", "서른세 갈래"),
    ("41개 구멍", "마흔한 개 구멍"),                # 21~99는 스물한·마흔한 (관형형)
    ("20개", "스무 개"),                          # 일의 자리가 0이면 스무
    ("194그루입니다.", "백구십네 그루입니다."),        # 100 이상은 한자어 + 고유어 일의 자리
]
...
def test_korean_skips_the_space_only_for_latin_units():
    """`3 m`은 단위지만 `1 모형`의 모형은 단위가 아니다 (unit_space: latin)."""
    assert spoken_line("지반에 3 m 박힌", "ko").spoken == "지반에 삼 미터 박힌"
    assert spoken_line("5분의 1 모형", "ko").spoken == "오 분의 일 모형"
...
def test_ambiguous_counters_are_left_alone_with_a_warning():
    ...
    assert "이십칠 단" in result.spoken   # 아는 것은 편다
