...
> **개정 (→ ADR-0038, 2026-08-19).** 스타일 문자열에서 **종이 절 둘을 걷었다** —
> `on white paper` 삭제, 가장자리 페이드 → 배경 채움. MJ가 종이를 **물체로 찍고**
> 있었다(4장 중 3장). 수채·흑연·펜 언더드로잉 절은 그대로다 — 그것이 스케치 느낌의
> 유일한 출처다. **자막 색도 뒤집었다**: 검정 글자를 고른 근거가 "베이스가 흰 종이라"였고
> 그 전제가 사라졌다.
- 날짜: 2026-08-11
