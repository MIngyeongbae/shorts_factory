...
`info` 씬에서 정보를 지는 것은 빨간 표시가 아니라 **그림 자체**다. 표시는 그 위의 주석일 뿐이다.

씬 계약에 `info.device`가 있으면 그 장치를 **SUBJECT 단락에서 실제로 무대에 세워라** — 문구를 옮겨 적는 것이 아니라, 그 장치가 성립하도록 무엇이 어디에 놓였는지 쓰는 것이다. 절단면이면 잘린 면이 바깥 면과 **다른 색·명도**로 보인다고 쓰고, 축척 인물이면 사람이 어디에 얼마나 작게 서 있는지 쓰고, 셀 수 있는 반복이면 같은 것이 몇 겹으로 늘어서는지 쓴다. 장치 문구는 아래 씬 계약에 씬마다 붙어 있다.

**표시를 빼도 그림이 설명해야 한다.** 검수에 걸려 RED 절이 떨어져 나가도 남는 그림이 여전히 그 씬의 `visual_goal`을 말하는가 — 그것이 이 절의 기준이다. 지금까지 우리 `info` 씬은 평범한 그림 위에 빨간 줄 하나였고, 그래서 표시가 떨어지면 아무것도 남지 않았다.

`info.device`가 비어 있는 씬은 지금까지대로 쓴다.
