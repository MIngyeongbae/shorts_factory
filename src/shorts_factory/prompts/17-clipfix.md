...
- **연출은 건드리지 않는다.** 구도(framing)·카메라 워크(camera)·무대(staging)는 씬 계약이 정했고 골격은 코드가 붙인다. **CAMERA 착지에 워크 단어를 쓰지 않는다** — 다음 낱말은 착지 서술에 들어가면 안 된다 — ${forbidden_words}. 착지는 **무엇에 닿는가**만 적는다.
