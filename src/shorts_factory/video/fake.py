...
테스트는 FFmpeg 없이 돈다 — `[7]`·`[9]`의 판단은 전부 명령·필터 그래프를 만드는 쪽에
있다 (정규화 길이·전환 종류·오프셋·트림 길이·자막 시각). 실제 인코딩이 확인해 주는 것은
"FFmpeg가 이 그래프를 받아들이는가" 하나뿐이라 그건 `--provider fake`로 배관을 관통할 때 본다.
...
class FakeFFmpeg:
    ...
    @property
    def vf(self) -> str:
        """`[7]`의 정규화·`[8]`의 엔딩 컷은 입력이 하나라 `-vf`를 쓴다."""
...
def write_fake_clips(
    ...
    """`[7. videogen]`이 놓을 자리에 클립 파일을 만든다 (specs/05 `clips/{scene_id}.mp4`).
