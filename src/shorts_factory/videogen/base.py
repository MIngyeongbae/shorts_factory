...
class VideoClient(ABC):
    ...
    #: 잡 id 입력 경로가 쓰는 선언. 텍스트→영상 어댑터는 비워 둔다.
    source_provider: str = ""

    #: **`first_frame`·`last_frame`을 실제로 쓰는가** (ADR-0070·0071). 기본은 거짓이다 —
    #: 텍스트→영상 어댑터는 프레임 필드가 있어도 무시한다. `[7]`은 프레임을 입력으로 받는
    #: 라인에서 이 값이 거짓인 어댑터를 만나면 **멈춘다**: 프레임을 실었는데 어댑터가
    #: 버리면 `[6]`이 만든 계측 표시가 화면에서 사라지고, 그 사실이 아무 데도 안 남는다.
