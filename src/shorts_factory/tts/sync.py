...
타임라인에 구멍이 생기고, `[7. videogen]`의 "클립 길이 = 씬 길이 + 디졸브 겹침 0.6초"와
...
def scale(
    ...
    오디오가 `factor`배 빨라지므로 타임스탬프는 `1/factor`배가 된다 (specs/05).
    `factor`는 **FFmpeg가 건 남은 몫**이다 (ADR-0081 결정 6 개정) — 엔진이 건 배속은
    정렬에 이미 실려 있다. 기본값 1.0에서는 이 함수가 아예 안 불린다 (ADR-0082).
