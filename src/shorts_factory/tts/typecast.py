...
- **배속은 엔진이 건다** (`output.audio_tempo` — ADR-0081 결정 6 개정). 정렬도 같은
  시간축으로 당겨져 오므로 `[3]`은 더 손대지 않는다. **기본값 1.0에서는 `audio_tempo`를
  아예 안 담는다** (ADR-0082) — 아래 `build_body`가 `tempo != 1.0`일 때만 넣는다
