...
> (`[1b]`·`[1c]`·`[2b]`·`[4]`·`[6r]`·`[8]`·`[10]`·`[11]`, 그리고 ADR-0032의 정책 대조).
...
=== 2부: 영상 생산 (MJ 정액 — 변동비 0, go 승인분만. ADR-0025) ===
...
- **[6. imagegen]**: 시작 전에 `style.dialect`와 프로바이더가 맞는지 대조하고 어긋나면 호출 없이 멈춘다 (ADR-0027). **같은 자리에서 `config/mj-proxy.policy.json`과 프록시 실제 상태를 대조하고, 어긋나도 호출 없이 멈춘다** (ADR-0032) — 방언이 틀리면 편당 과금을 태우고 정책이 틀리면 계정을 태운다. `--allow-policy-drift`로 명시하면 경고만 남기고 진행한다. **자동으로 고쳐 쓰지 않는다**; 맞추는 것은 `sf mj-policy apply`로 사람이 부른다. 씬당 1회 재시도. 2회 실패 시 인접 씬 이미지 재사용(카메라 워크만 변경)으로 폴백하고 리포트에 기록. 편당 베이스 호출 = 씬 수 (2-pass 없음, ADR-0019). **씬을 동시에 제출한다** (ADR-0031 §4). relax는 제출 후 대부분 기다리는 시간이라 한 줄로 세울 이유가 없다 — 27씬을 순차로 돌면 잡 하나가 100초여도 45분이 된다(실측). **워커 수는 여기에 적지 않는다.** `--jobs`가 없으면 프로바이더에게 묻고(`ImageClient.concurrency()`), MJ 어댑터가 활성 계정의 `relaxCoreSize`를 읽는다. 그 값은 **구독 플랜이 정하므로** 문서나 코드에 적어 두면 플랜을 바꾼 날 조용히 틀린다 (ADR-0031 G3, ADR-0032). 못 읽으면 예외를 올리지 않고 1로 떨어진다 — 느려질 뿐 틀리지 않는 쪽이다. **대기 상한도 여기에 적지 않는다** (ADR-0035). 잡 하나를 얼마나 기다릴지는 relax 큐가 정하고 그것을 아는 것은 프로바이더뿐이다 — 단계가 숫자를 선언하면 어댑터가 선언한 값이 죽고, 짧게 잡힌 상한은 **이미 성공한 잡을 버린 뒤 재시도로 큐를 두 배로 만든다**(실측). 사람이 정해야 하면 `--timeout`이 이긴다. `images.json` 갱신은 **직렬**이다 — 씬 하나가 끝날 때마다 쓴다는 계약(ADR-0020)의 목적이 재실행 시 다시 사지 않는 것이므로 병렬이 그것을 깨면 안 된다. 429(`ImageGenRateLimited`)가 나오면 워커를 1로 줄이고 백오프한다. **MJ가 낸 4장을 전부 `images/_cand/{scene_id}/q{0..3}.png`로 남기고** `q0`을 `images/{scene_id}.png`에 복사한다 — 하류 계약은 그대로다. 사분면을 고르는 것은 `[6r]` 몫이고, 이미 산 것이라 추가 과금이 없다.
...
옮겨 갔기 때문이다. 종량제 시절의 "편당 ~$7"은 폐기된 값이다.

| 자원 | 성격 | 한도 |
|---|---|---|
| **MJ relax** (이미지, `[6]`) | **추가 과금 없음.** `relax_credits`는 한도가 아니라 음수로 내려가는 사용량 기록이다 | 일일 제출 상한만 (`dayRelaxDrawLimit`) |
| **MJ fast** (`mj_video`, `[7]`) | 돈이 아니라 **GPU 시간.** 영상 1잡 = 8분 | 월 900분 → **월 11.2편** |
| Claude 구독 | 고정비. 1부 전 단계가 여기서 돈다 (ADR-0008) | 사용 한도 |
| ElevenLabs (`[3]`) | 플랜 문자 수 할당량 | 플랜별 |

- **씬 수가 늘어도 이미지 비용이 안 는다.** 이것이 ADR-0025의 목적이고, 편당 씬 수를
  아끼던 설계 압력이 사라진 자리다
- **실제로 편수를 묶는 것은 fast 시간이다.** `motion`이 `mj_video`인 씬이 많은 편은
  월 편수를 그만큼 깎는다 — `[1s]`가 영상 모션을 고를 때 쓰는 예산이 이것이다
