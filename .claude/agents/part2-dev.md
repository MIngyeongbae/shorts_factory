...
- 주로 볼 스펙: `specs/02`(씬 계약) `specs/03`(시각 룰) `specs/04`(오디오 룰) `specs/05`(파이프라인)
  ...
  0004+0081(TTS — 엔진은 어댑터 선택, 기본 타입캐스트) · 0002(2레이어 텍스트)
- **돈이 드는 API를 함부로 호출하지 마라.** 변동비는 라인의 것이다 — 로컬 GPU 라인은 0,
  유료 API 라인은 토픽당 ≈$8(쇼츠 1편당 ≈$2.7). TTS는 어느 제공자든 문자 과금이라
  (타입캐스트 크레딧 / ElevenLabs 문자 할당량) `[3]` 재실행도 돈이다. 실제 호출이 필요하면 **먼저 보고하고 승인을 받아라.** 개발은 목업·픽스처로 한다
